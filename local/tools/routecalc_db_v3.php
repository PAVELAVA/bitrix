...
function sanitize_addr(string $s): string {
    ...
    // Чуть помогаем геокодеру: если в строке нет упоминания России, добавляем её
    if (!preg_match('/Россия|Russian Federation|RU/i', $s)) {
        $s .= ', Россия';
    }
    return trim($s, " \t\n\r\0\x0B,");
}
...
function http_get(string $url, int $timeout = 15, array $headers = []): array {
    // Сначала HTTP/1.1 + IPv4 (то, что любит Nominatim), потом остальное
    $attempts = [
